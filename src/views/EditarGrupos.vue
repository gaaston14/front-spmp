...
export default {
  ...
  mounted() {
    this.numGrupo = this.$route.params.numGrupo;
    ...
    async fetchTecnicos(tecnicosIds) {
      try {
        const requests = tecnicosIds.map((idTecnico) =>
          axios.get(`http://localhost:4000/api/tecnicos/${idTecnico}`)
        );
    ...
    async fetchTecnicosDisponibles() {
      try {
        const response = await axios.get(
          "http://localhost:4000/api/tecnicos/libres"
        );
      ...
      if (tecnicoSeleccionado) {
        ...
        axios
          ...
          .then((response) => {
            console.log(response.data);
            this.mostrarMensajeEliminado();
            const tecnicosIds = [...this.$route.params.tecnicosIds, tecnicoSeleccionado.id]; // Agregar el ID del nuevo técnico al arreglo de IDs
            this.fetchTecnicos(tecnicosIds);
